...
Die Firma *High Five Company* soll für das Unternehmen *Mobiliar Versicherung* eine Softwarearchitektur zur Protokollierung von Abfragen und Mutation der Kundendaten erstellen.
// Diese Kundendaten sind vertraulich und es muss nachträglich festgestellt werden, zu welchem Zweck die Daten und von wem sie verwendet werden.
...
| Eine Anfrage muss in jedem Fall protokolliert werden. Ob die Anfrage fehlerhaft oder erfolgreich ist.
...
Von der Mobiliar Versicherung folgende Interessenvertreter einbezogen und müssen diese Dokument gutheissen:
...
* Performanz, Verfügbarkeit und Vollständigkeit der Protokollierungsdaten sind wichtige Architekturziele, darum werden sie mit 5 Punkten bewertet.
* Skalierbarkeit, Sicherheit, Batchprozesse und Integrierbarkeit werden mit 4 Punkten bewertet, weil sie integrale Bestandteile der Nicht-Funktionalen Anforderungen sind.
* Betriebs- und Wartungskosten, die Erfüllung der fachlichen Anforderungen sind wichtiger als die Kosten, daher werden die Kosten nur mit 2 bzw. mit 3 Punkten bewertet.
Da die Lebensdauer der Applikation mehrere Jahre dauert, werden die die laufenden Kosten höher bewertet.
* Die Software wird nicht ständig erweitert, somit wird die Erweiterbarkeit mit 3 Punkten bewertet.
* Das Know-How der Mitarbeiter bewerten wir mit 3 Punkten, weil wir nach Bedarf kurzfristig externe Ressourcen  einkaufen  oder mittelfristig die Mitarbeiter  ausbilden können.
* Die Konfigurierung der Aufbewahrungsfrist wird nicht häufig vorgenommen, somit bewerten wir sie mit 2 Punkten.
...
* ELK-Stack : Elasticsearch, Logstash und Kibana, Tool zum Auswerten von Logdateien. https://www.elastic.co/webinars/introduction-elk-stack
