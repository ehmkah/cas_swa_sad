...
Es wird eine neue Anwendung implementiert, die täglich die Daten der einzelnen Textdateien in einer zentralen Datenbank speichert.
...
Zusätzliche Anwendung notwendig.
...
Es wird ein DB-Job eingerichtet, der jede Nacht die Daten löscht, die älter als ein 1 Jahr sind.
...
Wenn sich der Zeitraum ändert, muss der DB-Job angepasst werden.
