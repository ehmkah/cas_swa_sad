...
||keine. Anforderung an die Verfügbarkeit unter Hochlast müssen gewährleistet sein (siehe "Qualitätsziele")

2+| *Software-Vorgaben*

| Application Server		| JavaEE7 Kompatibilität muss gewährleistet sein
| Datenbank                     | Oracle 12c
| Unterstützte Browser          | Chrome, Safari, IE, Firefox (mit Version neuer als 01.01.2012)
| Unterstützung Mobile Browser  | Zwingend Nutzung von __Responsive Design__

2+| *Vorgaben des Systembetriebs*

| Zugriff auf GUI für Verkäufer | Der Zugriff auf das GUI für Verkäufer ist nur aus dem Firmen-LAN gestattet

2+| *Programmiervorgaben*

| Programmiersprache | Java, mit Java-EE 7
|===
