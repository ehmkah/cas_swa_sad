...
Ein Performancesitzplatz besteht aus einem Sitzplatzstatus (frei, reserviert, verkauft) und einem Sitzplatz, damit ist eine platz-genaue Zuordnung von bereits reservierten, verkauften und freien Sitzplätzen möglich.
...
Dies erlaubt es, dass die Sitzplatzkategorie-Verkaufspreise verändert werden können, ohne dass bereits verkaufte Sitzplätze von der Preisänderung betroffen sind.
